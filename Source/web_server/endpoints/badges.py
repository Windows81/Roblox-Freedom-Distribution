...
def _(self: web_server_handler) -> bool:
    ...
    username = self.server.storage.players.get_player_field_from_index(
        self.server.storage.players.player_field.IDEN_NUM, user_id_num, self.server.storage.players.player_field.USERNAME)
    if not badge_data:
        self.send_data('0')
        return True
    if not username:
...
def _(self: web_server_handler) -> bool:
    ...
    database = self.server.storage.badges
    self.send_data("Success" if database.check(
        user_id_num, int(badge_id)) is not None else "Failure")
