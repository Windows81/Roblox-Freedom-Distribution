...
class database(_logic.sqlite_connector_base):
    TABLE_NAME = "devproducts"

    class field(enum.Enum):
        USER_IDEN_NUM = '"user_id_num"'
        DEVPRODUCT_ID = '"dev_product_id"'
    ...
    def first_time_setup(self) -> None:
        self.sqlite.execute(
            ...
            CREATE TABLE IF NOT EXISTS "{self.TABLE_NAME}" (
                ...
                {self.field.DEVPRODUCT_ID.value} INTEGER NOT NULL,
    ...
    def update(self, user_id_num: int, devproduct_id: int) -> None:
        self.sqlite.execute(
            f"""
            ...
            VALUES (?, ?, 1)
            ON CONFLICT({self.field.USER_IDEN_NUM.value}, {self.field.DEVPRODUCT_ID.value})
            DO UPDATE SET
    ...
    def receipts(self) -> list[tuple[int, int, str]]:
        ...
        return [
            (user_id_num, devproduct_id, f"{devproduct_id}-{user_id_num}",)
            for (user_id_num, devproduct_id) in passes
        ]
