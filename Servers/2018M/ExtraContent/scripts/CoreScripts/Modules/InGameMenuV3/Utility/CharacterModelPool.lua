...
local CLUSTER_COMPOSITION_TIMEOUT_MS = game:DefineFastInt("InspectAndBuyV2ClusterCompositionTimeoutMs", 3000)
...
local function setupCharacterModels(cframePos)
	local r6 = Players:CreateHumanoidModelFromDescription(Instance.new("HumanoidDescription"), Enum.HumanoidRigType.R6) :: HumanoidModel
	local r15 = Players:CreateHumanoidModelFromDescription(Instance.new("HumanoidDescription"), Enum.HumanoidRigType.R15) :: HumanoidModel
	...
	r15:PivotTo(r6.HumanoidRootPart.CFrame)
...
function CharacterModelPool.new(worldModelRef, cframePos, rigType, mockApplyDescription)
	...
	self.inProgressPromise = (nil) :: Promise<any>?
	-- The promise waiting to update the R15 avatar
	self.waitingPromise = (nil) :: Promise<any>?
	-- The promise currently updating the R6 avatar
	self.r6Promise = (nil) :: Promise<any>?
...
function CharacterModelPool:syncRotation(oldCharacter, currentCharacter)
	...
	local currentCFrameP = currentCharacter.HumanoidRootPart.CFrame.Position
...
local function cancelPromise(promise: Promise<any>?)
	if promise then
		promise:cancel()
	end
end
