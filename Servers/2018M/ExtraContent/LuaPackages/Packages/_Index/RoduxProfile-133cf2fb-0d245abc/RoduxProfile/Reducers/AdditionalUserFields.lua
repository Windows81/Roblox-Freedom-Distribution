--[[
	...
	Fields: isBanned and description
...
export type State = {
	-- maps user ID to a table of isDeleted, description user fields
	[string]: { isDeleted: boolean, description: string },
}
...
return function(options)
	...
	return Rodux.createReducer({}, {
		[NetworkingUsers.GetUserV2FromUserId.Succeeded.name] = function(state, action)
			...
			local user = {
				[userId] = { isDeleted = response.isBanned, description = response.description },
			}
