...
function tryHydrate(fiber, nextInstance)
  ...
  elseif fiber.tag == SuspenseComponent then
    if enableSuspenseServerRenderer then
      local suspenseInstance: nil | SuspenseInstance = canHydrateSuspenseInstance(
        nextInstance
      )
      if suspenseInstance ~= nil then
        ...
        local dehydratedFragment = createFiberFromDehydratedFragment(
          suspenseInstance
        )
...
function popToNextHostParent(fiber: Fiber)
  ...
  while
    parent ~= nil and
    parent.tag ~= HostComponent and
    parent.tag ~= HostRoot and
...
function popHydrationState(fiber: Fiber): boolean
  ...
  if
    ...
    (type_ ~= 'head' and
      type_ ~= 'body' and
      not shouldSetTextContent(type_, fiber.memoizedProps))
  then
    local nextInstance = nextHydratableInstance
