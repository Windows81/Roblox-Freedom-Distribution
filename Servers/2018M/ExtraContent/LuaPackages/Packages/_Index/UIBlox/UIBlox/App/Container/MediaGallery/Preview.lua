...
Preview.validateProps = t.strictInterface({
...
function Preview.getDerivedStateFromProps(nextProps, lastState)
...
function Preview:renderScroller(style)
...
function Preview:renderThumbnails(style)
	local selectedIndex = self.state.selectedIndex
	local animationOn = self.state.animationOn
...
function Preview:renderTimerBar(style)
...
function Preview:calcSizesFromWidth(containerWidth, numberOfThumbnails)
...
function Preview:calcSizesFromHeight(containerHeight, numberOfThumbnails)
...
function Preview:updateSizes(container)
