...
function BottomLockedScrollView:didUpdate(_, _)
	local scrollingFrame = self.scrollViewRef:getValue()
	self.canvasPositionAtBottom = math.max(self.canvasPositionAtBottom, scrollingFrame.CanvasPosition.Y)
	if self.currCanvasPositionY >= self.canvasPositionAtBottom then
		scrollingFrame.CanvasPosition = Vector2.new(0, MAX_NUM)
	end
end
