...
return function(options: roduxFriendsTypes.RoduxFriendsOptions)
	...
	return Rodux.createReducer(DEFAULT_STATE, {
		...
		[FriendsNetworking.GetFriendRecommendationsFromUserId.Succeeded.name] = function(state: roduxFriendsTypes.FriendshipStatus, action: roduxFriendsTypes.GetFriendRecommendationsFromUserId)
			local responseData = action.responseBody.data
			local recommendedUsers = {}
			for _, user in pairs(responseData) do
				recommendedUsers[tostring(user.id)] = Enum.FriendStatus.NotFriend
			end
			return llama.Dictionary.join(state, recommendedUsers)
		end,
