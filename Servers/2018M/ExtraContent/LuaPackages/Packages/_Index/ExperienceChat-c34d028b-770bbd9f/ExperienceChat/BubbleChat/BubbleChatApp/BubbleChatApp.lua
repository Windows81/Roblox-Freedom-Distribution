...
function BubbleChatApp:render()
	if not self.props.voiceEnabled and not self.props.isBubbleChatEnabled then
	...
	return Roact.createElement(
		...
		Dictionary.map(userIds, function(isVoiceParticipant, userId)
			...
			return self.props.isBubbleChatEnabled and Roact.createElement(BillboardGui, {
				userId = userId,
...
local function mapStateToProps(state)
	return {
		...
		voiceEnabled = state.Voice.isEnabled and state.BubbleChatSettings.Enabled,
		participants = state.voiceState or {},
