...
return function()
	...
	describe("WITH the given players", function()
		...
		store:dispatch(PlayerAdded(4, "Player4", "Player4+DN"))
		...
		it("SHOULD return a single unique player by display name with a unique display name", function()
			local result = getAutocompletePlayersFromString(players, "Player4+DN")
			expect(result).toEqual({
				{ username = "Player4", displayName = "Player4+DN", userId = "4" },
			})
		end)
