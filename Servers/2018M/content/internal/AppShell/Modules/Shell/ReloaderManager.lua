...
do
	...
	function ReloaderManager:addReloaderFunc(reloaderName, reloaderFunc, interval, userRelated)
		...
		reloaders[reloaderName][funcId] =
			{
				...
				suspend = false,
				mutex = false,
				lastUserState = userRelated and currentUserState
			};
		...
		spawn(function()
			while GetReloaderFuncObj(reloaderName, funcId) do
				...
				local lastUserState = ReloaderFuncObj.lastUserState
				local userRelated = ReloaderFuncObj.userRelated
				if not userRelated or lastUserState == currentUserState then
					ReloaderManager:callReloaderFunc(reloaderName, funcId, true)
				else
					ReloaderManager:removeReloaderFunc(reloaderName, funcId)
				end
			end
		end)
