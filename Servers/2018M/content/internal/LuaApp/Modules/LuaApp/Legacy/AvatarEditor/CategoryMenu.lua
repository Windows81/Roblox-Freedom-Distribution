...
function this:openTopMenu()
	...
	if LayoutInfo.isLandscape then
		...
		GetButtonClickEvent(closeButton):connect(function()
		  this:closeTopMenu()
		end)
	...
	for index, category in pairs(Categories) do
		...
		GetButtonClickEvent(categoryButton):connect(function()
		...
		if luaAppLegacyInputDisabledGlobally then
			-- CLIAVATAR-1289: If categoryButton.ZIndex is less than its container's ZIndex, Activated will not work
			categoryButton.ZIndex = categoryButton.Parent.ZIndex + 1
		end
	end
	if LayoutInfo.isLandscape then
...
return function(
	...
	GetButtonClickEvent(topMenuContainer):connect(function()
		this:openTopMenu()
	end)
