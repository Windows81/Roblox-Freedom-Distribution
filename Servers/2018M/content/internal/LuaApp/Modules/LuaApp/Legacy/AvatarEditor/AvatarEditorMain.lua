...
local UseAndroidBackButton = settings():GetFFlag("EnableAEAndroidBackButton")
...
local characterManager = CreateCharacterManager(
	...
	},
	{
		...
		CharacterR15New = sharedStorage:WaitForChild('CharacterR15New');
	},
...
function AppMain:Start()
	if inputBeganConnection == nil or not inputBeganConnection.Connected then
		inputBeganConnection = userInputService.InputBegan:connect(handleInput)
	end
	if inputChangedConnection == nil or not inputChangedConnection.Connected then
		inputChangedConnection = userInputService.InputChanged:connect(handleInput)
	end
	if inputEndedConnection == nil or not inputEndedConnection.Connected then
		inputEndedConnection = userInputService.InputEnded:connect(handleInput)
	end

	if touchSwipeConnection == nil or not touchSwipeConnection.Connected then
		touchSwipeConnection = userInputService.TouchSwipe:connect(function(swipeDirection, numberOfTouches, soaked)
	...
	-- Adding back button functionality for Android
	if UseAndroidBackButton then
		local success, res = pcall(function()
			return GuiService.ShowLeaveConfirmation:Connect(function()
				GuiService:BroadcastNotification("", NotificationType.BACK_BUTTON_NOT_CONSUMED)
			end)
		end)

		if success then
			BackButtonConnection = res
		end
	end
...
function AppMain:Stop()
	...
	if touchSwipeConnection ~= nil then
