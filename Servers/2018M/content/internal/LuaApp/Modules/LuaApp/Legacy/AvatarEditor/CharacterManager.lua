...
local AvatarEditorUseNewR15Model = Flags:GetFlag("AvatarEditorUseNewR15Model")
local AvatarEditorUseArtistIntentFolder = Flags:GetFlag("AvatarEditorUseArtistIntentFolder")
...
local useAnthroValues = Flags:GetFlag("UseAdvancedCharacterScales3")
...
return function(webServer, characterTemplates, defaultClothesIndex)
	...
	if not AvatarEditorUseNewR15Model then
		templateCharacterR15 = characterTemplates['CharacterR15']
	else
		templateCharacterR15 = characterTemplates['CharacterR15New']
	end
	...
	local function getChildrenOfModel(model)
		local children = {}
		...
		if not modelChildren then
			...
			for _, piece in next, modelChildren do
	...
	local function amendR15ForItemAddedAsModel(character, model)
		...
		-- Collect assets
		local childrenOfModel = {model}
		if childrenOfModel[1].ClassName == 'Model' then
			childrenOfModel = childrenOfModel[1]:GetChildren()
		end

		for _, childInModel in next, childrenOfModel do
			if AvatarEditorUseNewR15Model then
				if AvatarEditorUseArtistIntentFolder then
					if childInModel.Name:lower() == 'r15artistintent' then
						bodyStuff = getChildrenOfModel(childInModel)

						if not bodyStuff then
							error("R15ArtistIntent folder is empty on this model!")
						end
					elseif childInModel.className ~= 'Folder' then
						table.insert(otherStuff, childInModel)
					end
				else
					if childInModel.Name:lower() == 'r15fixed' then
						bodyStuff = getChildrenOfModel(childInModel)
					elseif childInModel.className ~= 'Folder' then
						table.insert(otherStuff, childInModel)
					end
				...
				if childInModel.Name:lower() == 'r15' then
					for _, piece in next, childInModel:GetChildren() do
		...
		if info then
			...
			if (not AvatarEditorSelectivelyUseDefaultAsset) or replaceParts then
				if info.replacesR15Parts then
					...
					if AvatarEditorUseNewR15Model then
						if AvatarEditorUseArtistIntentFolder then
							replaceFolder.Name = "R15ArtistIntent"
						else
							replaceFolder.Name = "R15Fixed"
						end
					end
