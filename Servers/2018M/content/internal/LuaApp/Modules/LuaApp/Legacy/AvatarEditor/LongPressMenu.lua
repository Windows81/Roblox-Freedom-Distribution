...
local GetButtonClickEvent = require(Modules.LuaApp.Legacy.AvatarEditor.GetButtonClickEvent)
local GetButtonDownEventConnected = require(Modules.LuaApp.Legacy.AvatarEditor.GetButtonDownEventConnected)
...
local function openMenu(title, tableOfButtons, assetId)
	...
	for i,v in pairs(tableOfButtons) do
		...
		if v.func then
			GetButtonClickEvent(button):connect(v.func)
		end
	end
...
return function(inCharacterManager, inShadeLayer, inMenuFrame, inDetailsMenuFrame)
	...
	GetButtonClickEvent(detailsCloseButton):connect(function()
		closeDetails()
	end)

	GetButtonDownEventConnected(detailsCloseButton, function()
		detailsCloseImageLabel.ImageTransparency = 0.25
	end)

	GetButtonClickEvent(menuCancelButton):connect(closeMenu)
	GetButtonClickEvent(shadeLayer):connect(function()
		closeMenu()
		closeDetails()
	end)
