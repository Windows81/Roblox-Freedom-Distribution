...
local function AboutTab(props)
	...
	for gameLayoutOrder, entry in ipairs(recommendedGames) do
		recommendedGameCards[tostring(entry.universeId) .. tostring(entry.isSponsored)] = Roact.createElement(GameCard, {
			entry = entry,
...
GameDetailsPage = RoactRodux.connect(function(store)
	...
	return {
		recommendedGames = state.GameSortsContents.Recommended
	}
end)(GameDetailsPage)
