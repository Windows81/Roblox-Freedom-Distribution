...
function InstancePicker:getPickerParent(instance: Instance?)
	-- The Universal App doesn't have a fixed ScreenGui
	local screenGui = instance and instance:FindFirstAncestorWhichIsA("ScreenGui")
	return screenGui or self.debugInterface.pickerParent or self:getRoot()
end
...
function InstancePicker:intersectMouse(root: GuiObject, mousePosition: Vector2)
	local descendants = root:GetDescendants()
	local result = reduce(descendants, function(current, instance: GuiObject)
		...
		-- Don't pick an instance if it is in a disabled screen gui
		local isDisabled = screenGui and screenGui.Enabled == false
		if intersects and not isDisabled then
	...
	if mousePosition ~= self.mousePosition then
		...
		if foundObject then
			if foundObject ~= self.selectedObject then
				self.pickerArea.Parent = self:getPickerParent(foundObject)
...
function InstancePicker:highlight(instance: GuiObject)
	-- Some instances may not have sizes so guard for error
	pcall(function()
		self.highlightArea.Parent = self:getPickerParent(instance)
