...
function GenericButton:init()
	self:setState({
		...
		absoluteSize = UIBloxConfig.genericButtonPassesMaxSizeToGenericTextLabel and
			Vector2.new(PLACEHOLDER_ABSOLUTE_SIZE_PX, PLACEHOLDER_ABSOLUTE_SIZE_PX) or
			nil,
	})
	...
	if UIBloxConfig.genericButtonPassesMaxSizeToGenericTextLabel then
	...
	return withStyle(function(style)
		...
		if isLoading then
			...
			if UIBloxConfig.genericButtonPassesMaxSizeToGenericTextLabel then
				...
				maxSize = Vector2.new(maxSizeX, maxSizeY)
			end
		...
		return Roact.createElement(Interactable, Cryo.Dictionary.join(self.props, {
			...
			[Roact.Change.AbsoluteSize] = UIBloxConfig.genericButtonPassesMaxSizeToGenericTextLabel and self.onAbsoluteSizeChanged or nil,
		}), {
