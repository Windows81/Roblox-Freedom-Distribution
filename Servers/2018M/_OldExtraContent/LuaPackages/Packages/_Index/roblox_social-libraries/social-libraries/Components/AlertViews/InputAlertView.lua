...
function InputAlertView:init()
	...
	self.updateModalPosition = function()
		...
		self:setState({
			keyboardOffset =
				...
				self.state.yPosition + self.state.height - UserInputService.OnScreenKeyboardPosition.Y or 0,
		})
	end
	...
	self.onHeightChanged = function(rbx)
		if not self.isMounted then
			return
		end

		if rbx.AbsoluteSize.Y ~= self.state.size then
			self:setState({ height = rbx.AbsoluteSize.Y })
		end
	end
...
function InputAlertView:render()
	...
	return Roact.createElement("Frame", {
		...
		[Roact.Change.AbsoluteSize] = self.onHeightChanged,
	}, {
