...
function story:init()
	...
	self.mockMessageHistory = {
		RBXGeneral = { "id1", "id2", "id3", "id4" },
	}
	local count = 0
	self.getMockMessageHistory = function()
		count = (count + 1) % (#self.mockMessageHistory.RBXGeneral + 1)
		return self.mockMessageHistory.RBXGeneral[count]
	end

	self.state.mockMessageHistory = {
		RBXGeneral = { self.getMockMessageHistory() },
	}
end
...
function story:didMount()
	...
	self.connection = self.props.timeSignal:connect(function(dt)
		...
		if timeElapsed >= frequency then
			...
			local RBXGeneral = List.append(self.state.mockMessageHistory.RBXGeneral, self.getMockMessageHistory())
			self:setState({
				mockMessageHistory = {
					RBXGeneral = RBXGeneral,
				},
			})
		end
	end)
