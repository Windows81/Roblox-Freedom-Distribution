...
return function()
	...
	describe("GIVEN timer", function()
		...
		it("SHOULD correctly fade in and out when moving over instance", function(c)
			local result = c:mount()
			-- Should be visible with inital transparency at startup
			local chatWindow = c.findFirstInstance(result.instance, { Name = "chatWindow" })
			local chatInputBar = c.findFirstInstance(result.instance, { Name = "chatInputBar" })
			local chatInputTextBox = c.findFirstInstance(chatInputBar, { Name = "TextBox" })
			local testComponents = { chatWindow, chatInputBar, chatInputTextBox }
	...
	describe("GIVEN text timer", function()
		beforeAll(function(c)
			c.timeBindableEvent = Instance.new("BindableEvent")
