...
local Roact = require(Root.Roact) :: any
local Cryo = require(Root.Cryo) :: any
local t = require(Root.t) :: any
local Otter = require(Root.Otter) :: any

local FitFrame = (require(Root.FitFrame) :: any).FitFrameOnAxis
...
Scroller.validateProps = t.interface({
	...
	-- Reference to the scrolling frame
...
function Scroller:didUpdate(previousProps, previousState)
	...
	if self.props.focusIndex ~= previousProps.focusIndex and
		self.props.focusLock ~= previousProps.focusLock then
			...
			self.log:trace("previousState.anchor.index {}", previousState.anchor and previousState.anchor.index or tostring(nil))
	end
...
-- Updates next state metadata, pools, and refpool
-- returns true if something is updated, false otherwise
function Scroller.updateMetadataList(metadata, pools, refpool, nextProps, trail, lead)
	local deletions = {}
	local additions = {}
	...
	for id, _ in pairs(metadata) do
		if not additions[id] then
			deletions[id] = true
		elseif metadata[id].name then
			additions[id] = nil
		end
	end

	if next(deletions) == nil and next(additions) == nil then
		return false
	end

	for id, _ in pairs(deletions) do
		Scroller.clearMetadata(metadata, id, nextProps)
	end

	for id, item in pairs(additions) do
		Scroller.updateMetadata(id, item, nextProps, metadata, pools, refpool)
	end

	return true
end

function Scroller.copyMetadataState(lastState)
	local nextMetadata = {}
	for id, meta in pairs(lastState.metadata) do
		nextMetadata[id] = meta
	end

	local nextPools = {}
	for class, pool in pairs(lastState.pools) do
		nextPools[class] = pool
	end

	local nextRefpool = {}
	for name, ref in pairs(lastState.refpool) do
		nextRefpool[name] = ref
	end

	return nextMetadata, nextPools, nextRefpool
end
...
function Scroller.getDerivedStateFromProps(nextProps, lastState)
	...
	local metadata, pools, refpool = Scroller.copyMetadataState(lastState)
	...
	if lastState.lastFocusLock ~= nextProps.focusLock then
		...
		if nextProps.animateScrolling and lastState.lastFocusLock ~= nil then
			...
			Scroller.updateMetadataList(metadata, pools, refpool, nextProps, nextProps.focusIndex, nextProps.focusIndex)
	...
	if anchorIndex and lastState.anchor.index == anchorIndex
		...
		and leadIndex and lastState.lead.index == leadIndex then
			...
			local updated = Scroller.updateMetadataList(metadata, pools, refpool, nextProps, lastState.trail.index, lastState.lead.index)
	...
	Scroller.updateMetadataList(metadata, pools, refpool, nextProps, newIndices.trailIndex, newIndices.leadIndex)
...
function Scroller:init()
	...
	self.paddingRef = Roact.createRef()
	...
	self.onScroll = function(rbx)
		...
		self.scrollDebounce = not self:isScrollingWithElasticBehavior()

		if not self:getRbx(self.state.trail.index) or not self:getRbx(self.state.lead.index) then
			self.scrollDebounce = false
			return
		end
...
function Scroller:getCurrentPadding()
	local pad = self.paddingRef.current
	if not pad then
		return 0
	end
...
function Scroller:findIndexAt(targetPos, hintIndex, extrapolate)
	...
	while currentDist ~= 0 and currentDist ~= nil  do
		if currentDist < 0 then
...
function Scroller:handleResize(size)
	if not self:getRbx(self.state.anchor.index) then
		return
	end
	...
	if self.relativeAnchorLocation >= frameSize * 0.5 then
		...
		if math.abs(botAdjustment) < math.abs(frameDiff) then
			self.anchorFramePosition = 0
		elseif newAnchor == self.state.anchor.index then
...
function Scroller.updateMetadata(id, item, props, metadata, pools, refpool)
	...
	if not meta.name then
		...
		meta.class = tostring(elem.component or elem.type)
		local pool = Scroller.getKeyPool(meta.class, pools)
		meta.name = pool:get()
	end

	if not refpool[meta.name] then
		refpool[meta.name] = Roact.createRef()
	end
	meta.ref = refpool[meta.name]
end

-- Clear the metadata for an element that is being unloaded.
function Scroller.clearMetadata(metadata, id, props)
	local meta = metadata[id]
	if not meta then
		return
	end
	...
	if not Cryo.List.find(props.recyclingDisabledFor, meta.class) then
		if meta.name then
			meta.name:release()
		end
	end
...
function Scroller.getKeyPool(class, pools)
	if not pools[class] then
		pools[class] = KeyPool.new(class)
	end
	return pools[class]
end
...
function Scroller:getMetadata(index)
	return self.state.metadata[self:getID(index)]
end
...
function Scroller:getRef()
	...
	return self.props.scrollingFrameRef or self._ref
end
